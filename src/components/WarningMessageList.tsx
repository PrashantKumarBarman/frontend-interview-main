...
const WarningMessageList: React.FC<MessageListProps> = (props: MessageListProps) => {
    let [count, setCount] = useState(0);

    useEffect(() => {
        let count = 0;
        props.messages.forEach((message) => {
            if(message.priority === Priority.Warn) {
                count++;
            }
        });
        setCount(count);
    ...
    return (
        <Grid item md={3} sx={{ m: 1 }}>
            ...
            { props.messages.map((message, index) => {
                if(message.priority === Priority.Warn) {
                    return (
                        <Card key={index} sx={{ maxWidth: '100%', bgcolor: 'warning.light', my: 1 }}>
